...
Download this repository as a ZIP file and unzip it. If you are using `git`, clone this repository.

In your local copy of this repository (the unzipped file you downloaded), navigate to the `scrollytelling/` directory.
...
7. **Fill out your sections as needed.** You can add a `title`, `description` which supports HTML tags. Also give each section a unique name in the section `id` property. If you have an image that goes with that section of the story, add the path to the image in the `image` property.

8. For `location`, you can use the `helper.html` file to help you determine the map's position. More below
...
- `scrollytelling`: Code for the vanilla version of the template
